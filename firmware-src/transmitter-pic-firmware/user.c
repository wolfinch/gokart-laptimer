...
void InitApp(void)
{
    ...
    TRISA       = 0x01;      // on PortA, set all o/p exect RA0         
    WPU         = 0xFF;      // Weak pullup for all pins in PORT-A
    PORTC       = 0x00;        
    PORTA       = 0x01;     // Set the init Pin Values
...
CCP1CON = 0b00110000 ;
    ...
    /* Interrupt config*/
    IOC 			= 0x01; //interrupt on change is enabled on RA0
